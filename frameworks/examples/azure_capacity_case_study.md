...
## Alternative D — Simulation-Optimization over Nested Policy Vectors

A genuinely different design (not offline-LP / online-controller). "Cheat question" caveat: Kaushik has built a version of the V1/V2 system before, so this is the angle to develop further as a distinct answer.

**Core idea:**
- Define a static admission **policy as a nested set of cumulative reservation thresholds** (X, Y, Z, W), e.g.: X% reserved for RI only; X+Y% for RI+Whale; X+Y+Z% for RI+Whale+Small-Paying; X+Y+Z+W% (≤100%) for everyone including Free Tier. Admission is then a simple threshold check against current occupancy — no eviction needed if RI's slice is never shared.
- This is structurally the **nested protection level / EMSR (Littlewood's rule)** family from airline revenue management, generalized to N classes.
- Each job type has its own **arrival process and residence-time (holding-time) distribution** — since jobs occupy capacity for a duration, this is closer to a **loss-network / multi-class queueing** problem than simple arrival-counting. Real distributions (heavy-tailed job durations) likely violate the Poisson/exponential assumptions behind closed-form Erlang-type approximations — so simulate directly rather than relying on an approximation of unknown quality.

**Simulation-Optimization search:**
- For a candidate policy vector (X,Y,Z,W) (constrained to the simplex X ≤ X+Y ≤ X+Y+Z ≤ X+Y+Z+W ≤ 1), run a discrete event simulation and record multiple metrics: revenue, SLA attainment per tier, eviction rate (if any).
- Naive grid search over 4D + replication-per-point gets expensive fast (noisy simulation outputs need multiple replications — a **ranking & selection** problem).
- Better: **Cross-Entropy Method** — sample a batch of policy vectors, simulate each, keep the **Pareto-non-dominated** ("elite") set across the metrics, then sample the next batch from a distribution concentrated around that elite/frontier region. Repeat — this both converges the policy search *and* refines the estimated Pareto frontier.
- The frontier itself is the deliverable for the "I" discussion: the business chooses where to sit (e.g., accept X% lower revenue for zero RI rejection), rather than the system claiming a single "optimal" answer.

**Online layer:** unchanged from Version 2 — admission control is a cheap threshold check against the current policy vector, re-derived periodically via the simulation-optimization loop.

**Compute cost:** each (policy vector × replication) simulation is independent — embarrassingly parallel, scales well across distributed/GPU compute. Worth a one-liner if asked "is this practical at scale."
